...
dll.ExecToTdx.restype=ctypes.c_char_p         # 在客户端调用功能或URL
dll.GetGPBlockInStr.restype=ctypes.c_char_p         # 获取股票所属板块
...
def convert_or_validate(data):
    ...
    if isinstance(data, list):
        ...
        for item in data:
            ...
            if not _is_valid_symbol_code(item):
                print(f"代码无效或暂不支持: {item}")
                return ""
...
def get_warn_struct_str(stock_list:        List[str] = [],
    ...
    获取预警结构字符串
    """
    # 1. 校验stock_list格式, 支持A股, 港股, 美股, 期货, 期权
    for stock in stock_list:
        if not _is_valid_symbol_code(stock):
            raise ValueError(f"Invalid stock code: {stock} (e.g. 688318.SH / 00700.HK / AAPL.US / CU2409.SHF)")
...
def check_stock_code_format(input_data):
    """
    校验单个代码或代码列表, 采用"代码.后缀"格式, 支持多市场
    :param input_data: str | list[str]
    """
    if not input_data:
        print("输入不能为空")
        return False
    ...
    for code in check_list:
        if not _is_valid_symbol_code(code):
            print(f"代码格式错误: {code}, 示例: 688318.SH / 00700.HK / AAPL.US / CU2409.SHF")
            return False
...
class tq:
    ...
    def _check_stock_code_format_batch(cls, stock_list):
        """批量校验代码格式, 支持A股, 港股, 美股, 期货, 期权"""
        return all(_is_valid_symbol_code(stock) for stock in stock_list)
        ...
        for stock in stock_list:
            try:
                ...
                if ptr and len(ptr) > 0:
                    data_dict = _json_loads_with_errorid_guard(ptr)
    ...
    def get_financial_data(cls,
        ...
        for stock in stock_list:
            try:
                ...
                # 获取所有列表的长度，检查是否一致
                list_lengths = [len(v) for v in data_dict['Value'].values()]
                if len(set(list_lengths)) != 1:
                    print("输入字典中各字段的列表长度不一致，返回当前数据。")
                    return data_dict['Value']
                
                # 2. 转换为DataFrame
                df = pd.DataFrame(data_dict['Value'])
    ...
    def get_financial_data_by_date(cls,
        ...
        for stock in stock_list:
            try:
                ...
                result_dict[stock] = data_dict['Value']
    ...
    def get_trackzs_etf_info( cls, zs_code: str = ''):
        ...
        code_str = zs_code.encode('utf-8')
        try:
            timeout_ms = 60000
            ptr = dll.GetTrackZsETFInfoInStr(cls._get_run_id(), code_str, timeout_ms)
            if len(ptr) > 0:
                result_str = ptr.decode('utf-8')
            else:
    ...
    def get_relation(cls,
        ...
        try:
            timeout_ms = 10000
            ptr = dll.GetGPBlockInStr(cls._get_run_id(), code_str, timeout_ms)
            if len(ptr) > 0:
                result_str = ptr.decode('utf-8')
            else:
    ...
    def exec_to_tdx(cls, url:str = ''):
        ...
        try:
            ...
            json_str = json.dumps(code_json, ensure_ascii=False)
            json_str = json_str.encode('utf-8')
            ptr = dll.ExecToTdx(cls._get_run_id(), json_str, timeout_ms)
            if len(ptr) > 0:
                result_str = ptr.decode('utf-8')
            else:
